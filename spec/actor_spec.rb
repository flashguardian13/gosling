...
describe Actor do
  before(:all) do
    ...
    @read_only_actor = Actor.new(@window)
    @parent = Actor.new(@window)
    @child = Actor.new(@window)
  end
  
  describe '#new' do
    it 'requires a Gosu::Window' do
      expect { Actor.new(@window) }.not_to raise_error
      expect { Actor.new() }.to raise_error
    end
  end
  
  it 'has a transform' do
    expect(@read_only_actor.transform).to be_instance_of(Transform)
  end
  ...
  it "knows if it has a particular child or not" do
    expect(@read_only_actor.has_child?(Actor.new(@window))).to be false
  end
  
  it "will never add any of its ancestors as children" do
    @parent.add_child(@child)
    expect { @child.add_child(@parent) }.to raise_error
  end
  
  it "will not add itself as its own child" do
    expect { @child.add_child(@child) }.to raise_error
  end
  ...
  describe '#draw' do
    ...
    context 'with children' do
      before(:all) do
        @child1 = Actor.new(@window)
        @child2 = Actor.new(@window)
