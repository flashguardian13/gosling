...
class Actor
  ...
  def parent=(parent)
    ...
    unless parent
      raise "You should use Actor.remove_child() instead of setting the parent directly." if @parent.has_child?(self)
    end
    @parent = parent
    if @parent
      raise "You should use Actor.add_child() instead of setting the parent directly." unless @parent.has_child?(self)
    end
  end
  
  def add_child(child)
    return if @children.include?(child)
    raise "An Actor cannot be made a child of itself." if child == self
    ancestor = parent
    until ancestor.nil?
      raise "Adding a child's ancestor as a child would create a circular reference." if child == ancestor
  ...
  def get_global_transform
    ...
    #~ current = self
    #~ while current do
      #~ transforms.unshift(current.transform.to_matrix)
      #~ current = current.parent
    #~ end
    
    #~ global_transform = Matrix.identity(3)
    #~ transforms.each { |tf| global_transform *= tf }
    #~ global_transform
  end
